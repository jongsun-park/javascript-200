class TodoApp {
  constructor(todos) {
    ...
    this.pushBtnEl = document.querySelector(".add-todo button");
    this.renderTodos();
    this.bindEvents();
  }
  ...
  createTodoEl(todo, id) {
    ...
    todoEl.innerHTML = `<input type="checkbox" ${
      todo.done ? "checked" : ""
  ...
  renderTitle() {
    ...
    if (this.titleEl) {
      this.titleEl.innerHTML = `${month}월 ${date}일 <span class="left-count">(${this.todoManager.leftTodoCount}개)</span>`;
    }
  }

  bindEvents() {
    this.pushBtnEl.addEventListener("click", (evt) => {
      if (
        (evt.target.nodeName =
          "INPUT" && evt.target.parentElement.className === "todo")
      ) {
